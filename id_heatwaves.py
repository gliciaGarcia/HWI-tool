...
def download_era5(
    ...
    """ Download ERA5 data.
    :param path: the path where the data must be saved.
    :type path: str
    ...
    list_ds = []
    for date in dates:
        filename = path + f't2m_max_era5_{date.strftime("%Y%m%d")}_p050.nc'
        ...
        if not os.path.exists(filename):
            ...
            request = {
                ...
                "year": list(map(str, dates.year.unique().to_list())),
                "month": list(map(str, dates.month.unique().to_list())),
                "day": list(map(str, dates.day.to_list())),
                "time": [
                    "18:00"
                ],
                'area': [
            ...
            client.retrieve(dataset_era5, request).download(filename)
            dataset = xr.open_dataset(filename).drop('number').rename({'valid_time': 'time'})
            data_daily = dataset.sel(time=dataset.time.dt.hour == 18) - 273.16

            data_daily.to_netcdf(filename)
            list_ds.append(data_daily)
    data_final = xr.concat(data_daily, dim='time')
...
def onda_de_calor(
    ...
    if len(list_filter) != 0:
        ...
        if len(list_datasets) != 0:
            ...
            print(f'Saving file in {file_out}')
...
def main():
    ...
    onda_de_calor(
        ...
        dir_out=f'{dir_local}/dados/saida_IDHW/'
    )
